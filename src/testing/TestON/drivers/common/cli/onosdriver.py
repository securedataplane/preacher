...
class OnosDriver( CLI ):
    ...
    def isup( self, node="", timeout=120 ):
        ...
        """
        try:
            self.handle.sendline( "/opt/onos/bin/onos-service server start& " + node )
            # self.handle.sendline( "ssh onos@" + node + " \"sh /home/onos/onos/tools/test/bin/start-onos-server.sh &>/dev/null &\"" )
            #self.handle.expect( "[1]" )
            # NOTE: this timeout is arbitrary"
            i = self.handle.expect(["[1]", pexpect.TIMEOUT], timeout)
    ...
    def detectionReport( self, nodeIp, searchTerms ):
        """
        Searches the latest ONOS log file for the given search terms and
        prints the total occurances of each term. Returns to combined total of
        all occurances.
        ...
        try:
            ...
            for termIndex in range( numTerms ):
                term = searchTerms[termIndex]
                logLines.append( [term] )
                cmd = "onos-ssh " + nodeIp + " cat /opt/onos/apache-karaf-3.0.5/data/log/karaf.log | grep " + term
                self.handle.sendline( cmd )
                self.handle.expect( "\$" )
                before = self.handle.before.splitlines()
                termsFound = []
                ...
                if term == "\"The average thoughput\"":
                    splitString = termsFound[ 0 ].split( "The average thoughput for" )
    ...
    def startCpuMemSampling( self, nodeIp ):
        """
        Start top to sample the cpu and memory usage of ONOS with SATS.
        """
        try:
            main.log.info( "Start top on the ONOS node" )
            cmd = "ssh onos@" + nodeIp + " \"sh ~/onos/tools/test/bin/start-onos-sampling.sh &>/dev/null &\""
            self.handle.sendline( cmd )
            self.handle.expect( "\$" )
            return main.TRUE
        except pexpect.EOF:
    ...
    def stopCpuMemSampling( self, nodeIp ):
        """
        Stop sample the cpu and memory usage of ONOS with SATS.
        """
        try:
            main.log.info( "Stop top on the ONOS node" )
            cmd = "ssh onos@" + nodeIp + " \" sh ~/onos/tools/test/bin/stop-onos-sampling.sh &>/dev/null &\""
            self.handle.sendline( cmd )
            self.handle.expect( "\$" )
            return main.TRUE
        except pexpect.EOF:
