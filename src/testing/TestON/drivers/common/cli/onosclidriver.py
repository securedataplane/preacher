    ...
    def setLogLevel( self, level="" ):
        ...
        try:
            ...
            if i == 2:
                self.handle.sendline( "" )
                self.handle.expect( "onos>" )
            self.handle.sendline( "log:set " + lvlStr )
            self.handle.expect( "onos>" )
...
##                self.handle.sendline( "" )
##                self.handle.expect( "onos>" )
            self.handle.sendline( "log:clear" )
            self.handle.expect( "onos>" )
    ...
    def getTrajectoryThroughput(self, searchTerms):
        """
            log:display and grep for the trajectory throughput results.
             Return the average throughput obtained.
        """
        ...
        try:
            ...
            for termIndex in range( numTerms ):
                ...
                cmd = "log:display | grep " + term
                self.handle.sendline(cmd)
                self.handle.expect("onos>")
                before = self.handle.before.splitlines()
                main.log.info(before)
                termsFound = []
                termsFound = before[2].split("is:")
                main.log.info(term + ":" + termsFound[1].strip())
                detectionResults.append(termsFound[1].strip())
                if term == "\"The average thoughput\"":
                    splitString = termsFound[0].split("org.onosproject.onos-app-tsamp - 1.4.1.SNAPSHOT")
                    splitStringAgain = splitString[1].split('for ')
                    threadCount = splitStringAgain[1].split(' threads')
                    # '[0m[36m2016-10-18 13:42:28,079 | INFO  | jectorySampling) | TrajectorySampling               | 1956 - org.onosproject.onos-app-tsamp - 1.4.1.SNAPSHOT | [43;30mThe average thoughput[m[0m[36m for 1 threads '
                    # threadCount = [int(s) for s in splitString[1].split() if s.isdigit()]
                    detectionResults.append(threadCount[0])
                    main.log.info(str(threadCount[0]) + " detectors")
    ...
    def isDropAttack( self ):
        ...
        """
        try:
            #dropValue="2016-03-02 20:29:20,377 | ERROR | ATS-Detector-0-0 | TrajectorySampling               | 185 - org.onosproject.onos-app-tsamp - 1.4.1.SNAPSHOT | ATS found a packet drop attack!"
            #cmd = "onos-ssh " + nodeIp + " cat \"" + dropValue + "\"> /opt/onos/apache-karaf-3.0.5/data/log/karaf.log"
            #self.handle.sendline( cmd )
            logLines = []
            cmd = "log:display"
            #logLines = self.sendline( cmd, showResponse=True )
            self.handle.sendline( "log:display | grep \"ATS found a packet drop attack\"" )
            self.handle.expect( "onos>" )
            logLines = self.handle.before.splitlines()
##            for line in logLines:
##                main.log.info( "logLines contains:" + str( line ) )
            if len( logLines ) > 3:
                if re.search( "attack", logLines[ 2 ] ):
##                    hashValueLine = []
##                    hashValue = []
##                    hashValueLine = logLines[ 2 ].split( "hash_value:" )
##                    hashValue = hashValueLine[ 1 ].split( "." )
##                    #ansi_escape = re.compile( r'\x1b[m' )
##                    #re.sub( r"\x1b.[^m]*m", '', hashValue )
##                    main.log.info( "Attack hashValue:" + hashValue[ 0 ] )
##                    hashValueInt = int( hashValue[ 0 ] )
##                    #return hashValue[ 0 ]
                    return main.TRUE
            else:
                return main.FALSE
...
##            return main.TRUE
    ...
    def isInjectionAttack( self ):
        ...
        try:
            logLines = []
            cmd = "log:display"
            self.handle.sendline( "log:display | grep \"ATS found a packet injection attack\"" )
            self.handle.expect( "onos>" )
            logLines = self.handle.before.splitlines()
            if len( logLines ) > 3:
                if re.search( "attack", logLines[ 2 ] ):
...
##                main.log.info( "logLines contains:" + str( line ) )
            if len( logLines ) > 3:
                if re.search( "attack", logLines[ 2 ] ):
                    ...
                    #ansi_escape = re.compile( r'\x1b[m' )
                    #re.sub( r"\x1b.[^m]*m", '', hashValue )
                    main.log.info( "Attacked hashValue:" + hashValue[ 0 ] )
                    hashValueInt = int( hashValue[ 0 ] )
                    return hashValue[ 0 ]
                    #return main.TRUE
            return 4096
...
##            return main.TRUE
    ...
    def getInjectionHashValue( self ):
        """
            log  the commands in the onos CLI.
            returns main.TRUE on success
            returns main.FALSE if Error occurred
    ...
    def checkLog( self, cmdStr, level="" ):
        """
            log  the commands in the onos CLI.
            returns main.TRUE on success
            returns main.FALSE if Error occurred
        ...
        try:
            ...
            if i == 2:
                self.handle.sendline( "" )
                self.handle.expect( "onos>" )
            self.handle.sendline( "log:display" )
            self.handle.expect( "onos>" )
