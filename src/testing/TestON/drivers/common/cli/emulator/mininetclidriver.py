...
class MininetCliDriver( Emulator ):
            ...
            for host in hostList:
                ...
                for temp in pingList:
                    ...
                    if re.search( ',\s0\%\spacket\sloss', response ):
                        ...
                        failedPings += 1
                    # Introduced a sleep of <wait> seconds as there is
                    # a problem with ONOS having a correct view of the
                    # flows installed
                    # time.sleep(wait)
                pingResponse += "\n"
    ...
    def arpHost( self, srcHosts ):
        ...
        import time
        try:
            main.log.info( "First have hosts populate their arp tables" )
            arpCmd = "py net.staticArp()"
            self.handle.sendline( arpCmd )
            self.handle.expect( "mininet>" )
            for srcHost in srcHosts:
                ...
                self.handle.sendline( cmd )
                time.sleep( 1 )
                self.handle.send( "\x03" )  # Send a ctrl-c to kill the arp
                self.handle.expect( "mininet>" )
            return main.TRUE
    ...
    def checkFlows( self, sw, dumpFormat=None ):
        if dumpFormat:
            ...
            command = "sh ovs-ofctl -O OpenFlow13 dump-flows " + str( sw )
        try:
            response = self.execute(
                cmd=command,
                prompt="mininet>",
    ...
    def checkSamplingFlows( self, sw ):
        try:
            for s in sw:
                cmd = "sh ovs-ofctl -O OpenFlow13 dump-flows " + str( s )
                self.handle.sendline( cmd )
                self.handle.expect( "mininet>" )
                flowTable = self.handle.before
                ...
                main.log.info( "Switch:" + str( s ) + " has" + str( len( testChksums ) ) + " sampling rules installed" )
                if len( testChksums ) == 19:
                    continue
                else:
                    return main.FALSE
##                    main.log.error( "No sampling flow for the following switch:" + str( s ) )
##                    main.log.error( "No point continuing" )
##                    main.cleanup()
##                    main.exit()
            return main.TRUE
        except pexpect.EOF:
    ...
    def checkAttackFlows( self, sw, searchString ):
        try:
            if searchString == "":
                searchString = "dl_src=00:00:00:00:00:02,dl_dst=00:00:00:00:01:02"
            attackFlowsExist = True
            flowExists = False
            for s in sw:
                flowExists = False
                cmd = "sh ovs-ofctl -O OpenFlow13 dump-flows " + str( s )
                ...
                for flow in flowTable:
                    main.log.info( "flow:" + str( flow ) )
                    if re.search( searchString, flow ):
##                        actions = []
##                        actions = flow.split( "actions=output:" )
##                        outputPort = actions[ 1 ]
##                        main.log.info( "got:" + str( actions[ 1 ] ) )
                        flowExists = True
                        break
                attackFlowsExist = flowExists and attackFlowsExist
##                main.log.info( "Switch:" + str( s ) + " has output port" + str( outputPort ) + " for " + searchString)
            if attackFlowsExist == True:
                main.log.info( "Attack flow exists" )
                return main.TRUE
            else:
                main.log.warn( "Attack flow does not exist" )
                return main.FALSE
        except pexpect.EOF:
            main.log.error( self.name + ": EOF exception found" )
            main.log.error( self.name + ":     " + self.handle.before )
    ...
    def getFlowOutputPort( self, s, matchOption="dl_src=00:00:00:00:00:02,dl_dst=00:00:00:01:00:02 actions=output"):
        try:
            outputPort = ''
            cmd = "sh ovs-ofctl -O OpenFlow13 dump-flows " + str( s )
            self.handle.sendline( cmd )
            ...
            for flow in flowTable:
                main.log.info( "flow:" + str( flow ) )
                if re.search( matchOption, flow ):
                    actions = []
                    actions = flow.split( "actions=output:" )
                    outputPort = actions[ 1 ]
                    main.log.info( "got:" + str( actions[ 1 ] ) )
                    break
            main.log.info( "Switches:" + str( s ) + " have output port for " + matchOption + " :" + str( outputPort ) )
            return outputPort
        except pexpect.EOF:
            main.log.error( self.name + ": EOF exception found" )
            main.log.error( self.name + ":     " + self.handle.before )
    ...
    def getFlowInputPort( self, s , matchOption="dl_src=00:00:00:00:00:02,dl_dst=00:00:00:01:00:02 actions=output"):
        try:
            inputPort = ''
            cmd = "sh ovs-ofctl -O OpenFlow13 dump-flows " + str( s )
            self.handle.sendline( cmd )
            ...
            for flow in flowTable:
                ...
                if re.search( matchOption, flow ):
                    inPort = []
                    inPortPartition = []
                    inPort = flow.split( "in_port=" )
                    inPortPartition = inPort[ 1 ].partition( "," )
                    inputPort = inPortPartition[ 0 ]
                    main.log.info( "got:" + str( inPortPartition[ 0 ] ) )
                    break
            main.log.info( "Switches:" + str( s ) + " have input port for " + matchOption + " :" + str( inputPort ) )
            return inputPort
        except pexpect.EOF:
            main.log.error( self.name + ": EOF exception found" )
            main.log.error( self.name + ":     " + self.handle.before )
    ...
    def parseFlowTable( self, flowTable, version="", debug=True ):
        ...
        for flow in flowTable:
            ...
            flag = {}
            if version == "1.3":
                if re.search("send_flow_rem", temp[index]) is not None:
                    # found the send_flow_rem flag
                    flag = {"flag": [temp[index]]}
                    index += 1
                else:
                    # did not find the send_flow_rem flag
                    # insert an empty flag list
                    flag = {"flag": []}
...
##                    cmd += " -F OpenFlow10-table_id"
                ...
                for r in response:
                   ...
                   # if re.search("send_flow_rem ", r) is not None:
                   #     # x = r.partition("send_flow_rem ")
                   #     # flow = x[0] + x[2]
                   #     # flows.extend( flow )
                   #      flows.extend( response )
                   # else:
                   #     # Missing send_flow_rem flag in the rule, bail.
                   #      return main.FALSE
    ...
    def addFlow( self, sw, version="", flowcmd="", debug=False ):
        '''
        Discription: Adds a flow to the flow table(s) on a switch or switches in a list.
            Each element is a flow.
        Arguments:
            Required:
                sw: The switch name ("s1") to retrive the flow table. Can also be
    ...
    def delFlow( self, sw, version="", flowcmd="", debug=False ):
        '''
        Discription: Adds a flow to the flow table(s) on a switch or switches in a list.
            Each element is a flow.
        Arguments:
    ...
    def checkFlowId( self, sw, flowId, version="1.3", debug=True ):
        ...
        try:
            ...
            if flows == main.FALSE:
                main.log.error("Problem in getFlowTable, could be the send_flow_rem flag is missing on the switch.")
                return main.FALSE
    ...
    def startTcpreplay ( self, host, pcapFileName, options="" ):
        """
        Start tcpreplay on a given host
        Mandatory:
            host - host name on which tcpreplay should be started
            pcapFileName - complete path and file name to pcap
            /home/mininet/ats-testing/pcapFile.pcap
        optional:
            options - The options for tcpreplay
        """
        try:
            cmd = 'tcpreplay '
            cmd += '-i ' + host + '-eth0 '
            cmd += options
            cmd += ' ' + pcapFileName
            #main.log.info( "cmd: " + cmd )
            self.handle.sendline( cmd )
            self.handle.expect( self.hostPrompt, timeout=3000 )
            main.log.info( self.handle.before )
            #main.log.info( "After: " + self.handle.after )
            return main.TRUE
        except pexpect.TIMEOUT:
            main.log.exception( self.name + ": Command timed out" )
            return main.FALSE
    ...
    def startAtsScapy( self, host, packets ):
        """
        Start the Scapy cli
        optional:
            mplsPath - The path where the MPLS class is located
        ...
        """
        try:
            self.handle.sendline( host + " scapy")
            self.handle.expect( self.scapyPrompt )
            self.handle.sendline( "conf.color_theme = NoTheme()" )
            self.handle.expect( self.scapyPrompt )
            main.log.info( "Scapy started" )
            for packet in packets:
                interface = host + "-eth0"
                scapyCmd = "sendp(" + packet + ", iface=\"" + interface + "\")"
                self.handle.sendline( scapyCmd )
                self.handle.expect( "Sent 1 packets" )
                main.log.info( self.handle.before )
            self.handle.sendline( "quit()" )
            main.log.info( "Scapy stopped" )
            self.handle.expect( "mininet>" )
            return main.TRUE
    ...
    def startScreenWithCommand( self, host, command, sessionId ):
        """
        Start a screen session on the host with the command
        E.g.:h1 screen -dmSL mininetSession top
        """
        screenCommand = " screen -dmSL " + sessionId + " " + command
        try:
            print "host is:" + host
            print "screenCommand:" + screenCommand
            self.handle.sendline( host + "ls -lrt")
            self.handle.expect( "mininet>" )
            main.log.info( "Screen session started" )
            return main.TRUE
    ...
    def setIptablesDrop( self, sw, interface, ip ):
        """
        Set an iptables drop rule on the given switch
        for traffic from given ip on given interface.
        @param sw
        @param interface
        @param ip
        """
        try:
            self.handle.sendline( sw + " iptables -i " + interface + " -A INPUT -s " + ip + " -j DROP")
            self.handle.expect( "mininet>" )
            main.log.info( "Drop rule inserted" )
            main.log.info( "Now verify it exists" )
            self.handle.sendline( sw + " iptables -L -n -v" )
            self.handle.expect( "DROP" )
            return main.TRUE
    ...
    def flushIptablesDrop( self, sw ):
        """
        Flush the iptables rules on the given switch
        @param sw
        """
        try:
            self.handle.sendline( sw + " iptables -F")
            self.handle.expect( "mininet>" )
            main.log.info( "iptables rules flushed!" )
            return main.TRUE
    ...
    def killScreens( self ):
        """
        Kill the screen session
        """
        try:
            self.handle.sendline( "sh screen -ls | grep Detached | cut -d . -f1 | awk '{print $1}' | xargs kill" )
            self.handle.expect( "mininet>" )
            main.log.info( "Killed the screen sessions." )
            # return main.TRUE
        except pexpect.TIMEOUT:
            main.log.exception( self.name + ": Command timed out" )
            return None
    ...
    def killTcpreplay( self ):
        """
        Kill the tcpreplay sessions
        """
        try:
            self.handle.sendline( "sh pkill -9 tcpreplay" )
            self.handle.expect( "mininet>" )
            main.log.info( "Killed the tcpreplay sessions." )
            # return main.TRUE
        except pexpect.TIMEOUT:
            main.log.exception( self.name + ": Command timed out" )
            return None
    ...
    def staticArpEntries( self ):
        """
        Set static ARP entries on the hosts from the mininet cli
        """
        try:
            self.handle.sendline( "py net.staticArp()" )
            self.handle.expect( "mininet>" )
            main.log.info( "Static arp entries set!" )
            return main.TRUE
