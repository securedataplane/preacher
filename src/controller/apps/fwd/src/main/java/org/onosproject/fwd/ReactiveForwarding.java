...
public class ReactiveForwarding {
    ...
    @Property(name = "matchIpv4Only", boolValue = false,
            label = "Enable matching only IPv4 packets; default is false")
    private boolean matchIpv4Only = false;
    ...
    @Property(name = "groupForwarding", boolValue = false,
            label = "Enable sampling via Group Tables for ATS; default is false")
    private boolean groupForwarding = false;

    @Property(name = "flowTableId", intValue = 1,
            label = "Flow table id to install the forwarding rules; default is 1")
    private int flowTableId = 1;
    ...
    private void readComponentConfiguration(ComponentContext context) {
        ...
        boolean matchIpv4OnlyEnabled =
                isPropertyEnabled(properties, "matchIpv4Only");
        if (matchIpv4Only != matchIpv4OnlyEnabled) {
            matchIpv4Only = matchIpv4OnlyEnabled;
            log.info("Configured. Matching IPv4 packet only is {}",
                     matchIpv4Only ? "enabled" : "disabled");
        }
        ...
        boolean groupForwardingEnabled =
                isPropertyEnabled(properties, "groupForwarding");
        if (groupForwarding != groupForwardingEnabled) {
            groupForwarding = groupForwardingEnabled;
            log.info("Configured. Group Forwarding for ATS is {}",
                     groupForwarding ? "enabled" : "disabled");
        }

        Integer flowTableIdConfigured =
                getIntegerProperty(properties, "flowTableId");
        if (flowTableIdConfigured == null) {
            log.info("Flow table id is not configured, default value is {}",
                     flowTableId);
        } else {
            flowTableId = flowTableIdConfigured;
            log.info("Configured. Flow Table id is configured to {}",
                     flowTableId);
        }
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            if (groupForwarding) {
                log.info("Woohoo, At least it can be enabled");
            }
            ...
            log.info("In fwd:   Going to installRule(contex, path.src.port)", appId.id());
    ...
    // Returns a deterministic path for a given set of paths.
    private Path pickForwardPathIfPossible(Set<Path> paths, PortNumber notToPort) {
        // Save the Set<Path> as an ArrayList<Path>
        ArrayList<Path> pathList = new ArrayList<Path>();
        // Save the deviceIds of the path in a list in the same order as the pathList
        ArrayList<String> pathDevicesList = new ArrayList<String>();
        // Use a Map to map the pathDevicesList to its path.
        Map<String, Path> devicePathMap = new HashMap<String, Path>();
        // Final sorted path list.
        ArrayList<Path> sortedPathList = new ArrayList<Path>();
        String pathDevices = new String();
        pathList.addAll(paths);
        /*
         * Iterate through the pathList to get the deviceIds via the links.
         * Store that in the pathDevicesList
         */
        for (Path path: pathList) {
            pathDevices = "";
            boolean firstLink = true;
            for (Link linkInPath : path.links()) {
                log.debug("pickForwardPathIfPossible: The link is: {}", linkInPath.toString());
                if (firstLink) {
                    String srcDevice = linkInPath.src().deviceId().toString();
                    String dstDevice = linkInPath.dst().deviceId().toString();
                    pathDevices += srcDevice;
                    pathDevices += dstDevice;
                    firstLink = false;
                } else {
                    String dstDevice = linkInPath.dst().deviceId().toString();
                    pathDevices += dstDevice;
            ...
            pathDevicesList.add(pathDevices);
        }
        log.debug("pickForwardPathIfPossible:pathDevicesList:{}", pathDevicesList.toString());
        /*
         * Map the deviceIds to its path.
         */
        for (int i = 0; i < pathDevicesList.size(); i++) {
            devicePathMap.put(pathDevicesList.get(i), pathList.get(i));
        }
        // compareDeviceId sorts the devices Ids in ascending order.
        Comparator<String> compareDeviceId = (id1, id2) -> id1.compareTo(id2);
        Collections.sort(pathDevicesList, compareDeviceId);
        /*
         * Now use the sorted pathDevicesList to get the path and store
         * in the sortedPathList.
         */
        for (String p : pathDevicesList) {
            sortedPathList.add(devicePathMap.get(p));
        }
        log.debug("pickForwardPathIfPossible:sortedPathList is:{}", sortedPathList.toString());
        Path lastPath = null;
        for (Path path : sortedPathList) {
            lastPath = path;
            if (!path.src().port().equals(notToPort)) {
                log.debug("pickForwardPathIfPossible:Going to return path:{}", path.toString());
                return path;
            }
        }
        log.debug("pickForwardPathIfPossible:Going to return lastPath:{}", lastPath.toString());
        return lastPath;
    }
    ...
    private void installRule(PacketContext context, PortNumber portNumber) {
        ...
        //TODO: The IPv4Only selector need to be done better.
        TrafficSelector.Builder selectorBuilderIPv4Only = DefaultTrafficSelector.builder();
        selectorBuilderIPv4Only.matchEthType(Ethernet.TYPE_IPV4);
        ...
        if (matchDstMacOnly) {
            ...
            log.info("Not matchDstMacOnly. Going to call selectorBuilder now.");
        ...
        Long portAsLong = portNumber.toLong();
        int portAsint = portAsLong.intValue();
        if (groupForwarding) {
            GroupId groupId = new DefaultGroupId(portAsint);
            log.info("Going to create a TrafficTreatment object now", appId.id());
            TrafficTreatment treatment = DefaultTrafficTreatment.builder()
                    .group(groupId)
                    .setOutput(portNumber)
                    .build();
            log.info("Going to create a ForwardingObjective now", appId.id());
            ForwardingObjective forwardingObjective = DefaultForwardingObjective.builder()
                    ...
                    .add();
            log.info("Going to forward using flowObjectiveService.forward now", appId.id());
            flowObjectiveService.forward(context.inPacket().receivedFrom().deviceId(),
                                         forwardingObjective);
        } else {
            log.info("Going to create a TrafficTreatment object now", appId.id());
            if (flowTableId > 0) {
                TrafficTreatment treatmentGoodTable = DefaultTrafficTreatment.builder()
                    .transition(flowTableId)
                    .build();

                TrafficTreatment treatmentAttackTable = DefaultTrafficTreatment.builder()
                    .transition(1)
                    .build();
            /*
             * Sampling rules point to GoodTable(2). In table(0)
             * send non-sampled packets to AttackTable(1).
             * In AttackTable(1), install a default flow rule to goto GoodTable(2).
             */
                log.info("In table(0) send IP packets to AttackTable(1)", appId.id());
                log.info("Going to create a ForwardingObjective now", appId.id());
                ForwardingObjective forwardingObjective = DefaultForwardingObjective.builder()
                        .withSelector(selectorBuilderIPv4Only.build())
                        .withTreatment(treatmentAttackTable)
                        .withPriority(flowPriority)
                        .withFlag(ForwardingObjective.Flag.VERSATILE)
                        .fromApp(appId)
                        .makeTemporary(flowTimeout)
                ...
                log.info("In AttackTable(1), send non-matching pkts to GoodTable(2)", appId.id());
//                log.info("Going to create a ForwardingObjective now", appId.id());
//                ForwardingObjective forwardingObjective = DefaultForwardingObjective.builder()
//                        .withSelector(selectorBuilderIPv4Only.build())
//                        .withTreatment(treatmentGoodTable)
//                        .withPriority(flowPriority)
//                        .withFlag(ForwardingObjective.Flag.VERSATILE)
//                        .fromApp(appId)
//                        .makeTemporary(flowTimeout)
...
//                flowObjectiveService.forward(context.inPacket().receivedFrom().deviceId(),
//                        forwardingObjective);
                log.info("Going to forward using flowRuleService.applyFlowRules now", appId.id());
            ...
            TrafficTreatment treatmentPort = DefaultTrafficTreatment.builder()
                    .setOutput(portNumber)
                    .build();
            log.info("Going to forward using flowRuleService.applyFlowRules now", appId.id());
            FlowRule f = DefaultFlowRule.builder()
                    .forDevice(context.inPacket().receivedFrom().deviceId())
                    .withSelector(selectorBuilder.build())
                    .withTreatment(treatmentPort)
                    .withPriority(flowPriority)
                    .forTable(flowTableId)
                    .makeTemporary(flowTimeout)
                    .fromApp(appId)
                    .build();
            flowRuleService.applyFlowRules(f);
